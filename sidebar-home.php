...
<?php if(!get_theme_mod( 'hide_default_menu', false ) || is_active_sidebar( 'left-widget-area' ) || is_active_sidebar( 'center-widget-area' ) || is_active_sidebar( 'right-widget-area' )): ?>
...
<aside id="secondary" class="widget-area" role="complementary">
    ...
    <?php if ( ! dynamic_sidebar( 'left-widget-area' ) ){
        ...
        //Default Welcart Archive Widget
        $args = array(
			'before_widget' => '<section id="widget_archive" class="widget widget_archive">',
			'after_widget' => '</section>',
			'before_title' => '<h3 class="widget_title">',
			'after_title' => '</h3>',
		);
        $widget_archive =array(
			'title' => 'アーカイブ',
			'icon' => 1,
		);
        the_widget( 'WP_Widget_Archives', $widget_archive, $args );
