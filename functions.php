...
function welcart_basic_widgets_init() {
    ...
    register_sidebar(array(
		...
		'description' => __( 'Widget area of archive', 'welcart_basic' ),
		'before_widget' => '<section id="%1$s" class="widget %2$s">',
		'after_widget' => '</section>',
		'before_title' => '<h3 class="widget_title">',
		'after_title' => '</h3>',
...
function exclude_category( $query ) {
  if ( $query->is_main_query() && !$query->get('category_name') == 'item' && !$query->is_search() ) { //商品ページ & 検索には商品を表示
    $cat_id = get_category_by_slug( 'item' );
    $id = $cat_id->term_id;
    $idString  = "-".$id;
    $query->set( 'cat', $idString );//マイナスをつけてカテゴリIDを除外する
  }
}
